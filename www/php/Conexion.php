...
class Usuarios{
    ...
    function conectarBD(){ 
            ...
            $usuario = "u251639074_admin";
            $pass = "cFt3,45SdX3rW.4F7Sdf";
            $BD = "u251639074_main";
    ...
    function createUser($ID_Maquina,$Descripcion,$Estado){
        //creamos la conexión
        $conexion = $this->conectarBD();
        //Escribimos la sentencia sql necesaria respetando los tipos de datos
        $sql = "insert into Maquinas (ID_Maquina,Descripcion,Estado) 
        values (".$ID_Maquina.",'".$Descripcion."','".$Estado."');";
        //hacemos la consulta y la comprobamos 
        $consulta = mysqli_query($conexion,$sql);
        if(!$consulta){
            echo "No se ha podido insertar una nueva Maquina en la base de datos<br><br>".mysqli_error($conexion);
        }
